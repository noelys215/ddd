...
export function CaseStudySection({
  ...
  return (
    <section
      aria-labelledby={id}
      className={`border-t border-white/10 py-14 md:py-20 ${className}`}
    >
      <SectionHeading
        symbol="cross"
        headingId={id}
...
export function MediaFrame({
  ...
  return (
    <figure className={`min-w-0 ${className}`}>
      ...
      {caption ? (
        <figcaption className="mt-3 text-xs leading-relaxed text-white/55">
          {caption}
        </figcaption>
      ) : null}
    </figure>
  ...
  return (
    <article className="grid items-center gap-8 border-b border-white/10 py-10 last:border-b-0 md:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] md:gap-12 md:py-14">
      <div className={reverse ? "md:order-2" : ""}>
        ...
        <p className="mt-5 border-l border-pink-500/60 pl-4 text-xs leading-relaxed text-white/55 md:text-sm">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] sm:items-center">
        {items.map((item, index) => (
          <div key={item} className="contents">
            <div className="rounded border border-white/15 bg-black/40 px-3 py-3 text-center text-xs leading-snug text-white/75">
              {item}
            </div>
  ...
  return (
    <figure className="mx-auto mt-10 max-w-3xl" aria-labelledby="architecture-caption">
        ...
        <div className="rounded border border-white/15 px-4 py-5">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            {services.map((service) => (
              <span
                key={service}
                className="rounded border border-white/10 bg-white/[0.03] px-2 py-3 text-center text-xs leading-snug text-white/70"
              >
                {service}
              </span>
            ))}
      ...
      <figcaption
        ...
        className="mt-4 text-center text-xs leading-relaxed text-white/55"
      >
    ...
    <div className="divide-y divide-white/10 border-y border-white/10">
      {notes.map((note) => (
        <details key={note.title} className="group">
          <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-4 py-4 text-left text-sm font-semibold text-white marker:content-none focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-pink-400 md:text-base">
